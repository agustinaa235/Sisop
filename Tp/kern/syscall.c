...
int next1 = 1;
/*
 * funcion que deuveleve un numero random
 */
unsigned int
rand2()
{
...
/*
 * Se encarga de cambiar la prioridad al proceso que llega.
 * En caso de que no exista devuelve -E_BAD_ENV. Si no tiene permisos
 * o si la prioridad es negativa devuelve -E_INVAL.
 */
static int
sys_change_priority(int pr, envid_t envid)
{
	struct Env *e;
	if ((envid2env(envid, &e, 0)) < 0) {
		return -E_BAD_ENV;
	}
	// Chequeo de permisos para cambiar prioridad
	...
	// si la prioridad que llega es mayor a la que el proceso tiene este no cambia de prioridad
	if (pr < e->priority) {
		e->priority = pr;
	}
	return 0;
